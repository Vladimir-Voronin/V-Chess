...
const right_menu_view = new RightMenuView([notation_bind]);
...
function raise_game_end_panel(is_draw, is_white_won, new_player_rating, rating_change) {
    ...
    close_end_game_panel.addEventListener("click", () => {
        outer.remove();
    });
...
let date_info = null;
let block_white = null;
let block_black = null;
online_game_socket.onmessage = function (e) {
    ...
    switch (data["type"]) {
        case "update_position":
            ...
            if (data["last_move_datetime"] && is_last_move_white) {
        ...
        case "game_end":
            ...
            if (block_black) {
                raise_game_end_panel(data["is_draw"], data["is_white_won"],
                    Math.round(data["new_white_rating"]), Math.round(data["white_rating_change"]));
            }

            if (block_white) {
                raise_game_end_panel(data["is_draw"], data["is_white_won"],
                    Math.round(data["new_black_rating"]), Math.round(data["black_rating_change"]));
            }
